/*	ESTE PROGRAMA TRATA DE ENCONTRAR LA M�QUINA DE TURING QUE ESCRIBA EN UNA
...
class Kolmogorov {
  ...
  static double	Pc,	Pm;
  static double	fTmp,W;					// WW --> Ponderaci�n para Solos,Pares,Tr�adas,Cuartetas
  ...
  static Random RandN;
//
  static int maxN=250,minN=1;			// N�mero de individuos
  static int maxNN=5000000,	minNN=1;	// N�mero de transiciones
  static int maxL=50000, minL=1;		// Longitud de la cinta
  static double	maxPc=1f, minPc=.01f;	// Probabilidad de cruza
  static double	maxPm=1f, minPm=.001f;	// Probabilidad de mutaci�n
  static int maxG=10000, minG=1;		// N�mero de generaciones
 ...
 */
  ...
  public static	void GetParams() throws	Exception {
	  ...
	  W=Double.valueOf(LHT(Fbr)).floatValue();	// 7) Ponderaci�n
  }//endGetParams

  public static	void UpdateParams()	throws Exception {
    PrintStream Fps=new PrintStream(new FileOutputStream(new File("AGParams.txt")));
    Fps.println(N+"\t\t1) Individuos");
    Fps.println(NN+"\t\t2) N�mero de transiciones");
...
/*
 *		La cinta inicial est� Llena de 0s
 */
 ...
 */
  public static	void Selecciona(double fitness[],String	genoma[]) {
	...
	public static void ShowTM(String STM) throws Exception {
		/*
		 *	LEE TODOS LOS BYTES DE LA M�QUINA DE TURING
		 *
		 */

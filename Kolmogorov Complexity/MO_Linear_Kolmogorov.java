 ...
 */
		...
		Estado = new boolean[64];
// 		Estado[0]=true;
		NewTape=UTM_AG.NewTape(genoma[i],Tape,NN,PP);
		int NumEstados=1;
		for (int l=1;l<64;l++) if (Estado[l]) NumEstados++;
		MaxEstados=Math.max(MaxEstados,NumEstados);
//		System.out.println("Ind: "+i+" Estads: "+NumEstados);
 		int Solos=0,Pares=0,Triadas=0,Cuartetas=0,Quintetas=0,Sextetas=0;
		double O1=0, O2=0, O3=0, O4=0, O5=0, O6=0, O7= NumEstados/MaxEstados;
		boolean FF2=true, FF3=true, FF4=true, FF5=true, FF6=true;;
		...
		if (NewTape.length()!=0){
			...
			for (int j=0;j<TgtLen;j++){
				...
				if (FF3) O3=(Triadas-2)/3;
				if (FF4){
					try{if (NewTape.substring(k,k+4).equals(TT.substring(j,j+4))) Cuartetas++;}
					catch (Exception e) {FF4=false;}
				}//endIf
				if (FF4) O4=(Cuartetas-3)/4;
				if (FF5){
					try{if (NewTape.substring(k,k+5).equals(TT.substring(j,j+5))) Quintetas++;}
					catch (Exception e) {FF4=false;}
				}//endIf
				if (FF5) O5=(Quintetas-4)/5;
				if (FF6){
					try{if (NewTape.substring(k,k+6).equals(TT.substring(j,j+6))) Sextetas++;}
					catch (Exception e) {FF4=false;}
				}//endIf
				if (FF6) O6=(Sextetas-5)/6;
			}//endFor
			double Min=Math.min(O1,O2);  Min=Math.min(Min,O3);
			       Min=Math.min(Min,O4); Min=Math.min(Min,O5);
			       Min=Math.min(Min,O6); Min=Math.min(Min,O7);
//			double Min=Math.min(O1,O7);			// Solo bits y tamaño
			fitness[i]=Min;						// Todos
//			fitness[i]=O1;						// Solo bits
		}else{
			fitness[i]=0d;
		}//endIf
	}//endFor
...
/*		Selecciona los N individuos con fitness MAXIMO
 *
 */

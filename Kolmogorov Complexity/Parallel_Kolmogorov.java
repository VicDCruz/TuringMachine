 ...
 */
 ...
 public static	void UpdateParams()	throws Exception {
  ...
  public static double[][] prepareData(int k) {
    double [][] auxTrain = new double[Archivo.N][Archivo.mO];
    ...
    for (int i = 0; i < nT; i++) {
      ...
      for (int j = 0; j < Archivo.mO; j++) {
        if (j < nN) {
          ...
          auxTrain[i][j] = numeric[i][j];
        } else {
          train[i][j] = catToNum[i][aux];
          auxTrain[i][j] = catToNum[i][aux];
          aux++;
        }//endIf
      }//endFor
    }//endFor
 ...
 *  public static void Evalua(double fitness[], String genoma[]) throws Exception {
    tasks.clear();
    for (int i = 0; i < N; i++) {             // Fill the N individuals to the Callable list.
      GetFenotiposOfGenoma(i);                // Get the decimal representation of the Genome
      double [][] NNdata = prepareData(i);    // Get the data corresponding to a specific individual
      BMLP neuralNet = new BMLP(NNdata, i);   // Create an object of a BMLP with the data retrieved before.
      tasks.add(neuralNet);                   // Add it to the list
    }//endFor

    // Prueba para revisar que el orden del iterador en efecto no es el orden
    // en el que se agregaron las redes. 
    // Iterator iter = tasks.iterator();
    // int k;
    // while(iter.hasNext()){
    //   BMLP net = (BMLP)(iter.next());
    //   System.out.println(net.id);
    // }

    try {
      // Call all the Individuals (Neural Networks) and wait for the response
      long t = System.currentTimeMillis();
      outputs = es.invokeAll(tasks);
      System.out.println("Cada GeneraciÃ³n tarda: " + (System.currentTimeMillis() - t));
      System.out.println("Outputs.size: " + (outputs.size()));
      for (int i = 0; i < outputs.size(); i++) {
        // La lista outputs estÃ¡ en el mismo orden que el iterador por default
        // del Set tasks. Esto es: el elemento i no corresponde a la red i. 
        double[] bmlp_out = (double[]) outputs.get(i).get();
        // Obtiene el ide de la red que se estÃ¡ corriendo en este momento
        int id = (int) bmlp_out[3];
        // Asigna los resultados al id correspondiente
        fitness[id] = bmlp_out[0];
        rms[id] = bmlp_out[1];
        AvgErr[id] = bmlp_out[2];
        
        System.out.println("i: " + id + " -> " + fitness[id]);
      }

    } catch (InterruptedException e) {
      System.out.println("ExecutorService Error: " + e);
    }
  }//endEvalua
*/
